...
if [[ ${#missing_hyprpm_deps[@]} -gt 0 ]]; then
    ...
    yay -S --needed --noconfirm "${missing_hyprpm_deps[@]}" || {
...
required_packages=(
    "hyprland" "hypridle" "hyprlock" "mako" "swww" "wl-clipboard"
    "cliphist" "fuzzel" "foot" "jq" "wf-recorder" "slurp" "libnotify" "fcitx5"
    "cmake" "meson" "cpio" "pkg-config" "git" "gcc" "tomlplusplus"
)
...
- **hyprpm Dependencies**: `cmake`, `meson`, `cpio`, `pkg-config`, `git`, `gcc`, `tomlplusplus`
...
7. Verify `hyprpm` dependencies: `cmake`, `meson`, `cpio`, `pkg-config`, `git`, `gcc`, `tomlplusplus`.
