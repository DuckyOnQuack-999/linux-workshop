...
# Hyprland Advanced Extras - Merged Installer
# Source baseline: Duckydots-v0.5.sh (robust), integrated with unique items from hyprland-advanced-extras.sh
# Goal: Fully working, no omissions, no missing logic, favor larger/robust implementations where overlap exists
...
# Track this installer path for use by reset scripts
INSTALL_SCRIPT_REALPATH="$(python3 - << 'PY'
import os, sys
try:
...
PY
" "$0")"
INSTALL_SCRIPT_PATH_FILE="$HOME/.config/hypr/install_script_path"
mkdir -p "$HOME/.config/hypr"
if [[ -n "${INSTALL_SCRIPT_REALPATH}" ]]; then
    echo "$INSTALL_SCRIPT_REALPATH" > "$INSTALL_SCRIPT_PATH_FILE"
fi
...
$content
EEOF
    # Mark scripts executable; configs will also get +x which is harmless
...
# Install core packages (union of both scripts) in parallel
...
# Install potentially conflicting packages (with fallback)
...
# Install pywal (ensure availability for scripts)
log_info "Ensuring pywal via pipx..."
pipx install pywal || {
    log_error "Failed to install pywal via pipx"
    exit 1
} &
...
# Reset Hyprland configuration (reads installer path from file written above)
...
if [[ $response -eq 0 ]]; then
    reset_config
    if [[ -f "$INSTALL_SCRIPT_PATH_FILE" ]]; then
        INSTALL_SCRIPT="$(cat "$INSTALL_SCRIPT_PATH_FILE")"
    else
        INSTALL_SCRIPT=""
    fi
    if [[ -n "$INSTALL_SCRIPT" && -x "$INSTALL_SCRIPT" ]]; then
        ...
        echo "Warning: Installation script path not found or not executable."
        echo "You can manually run your installer once ready."
    fi
else
...
# Advanced Hyprland Features Manager (merged)
...
create_script "$HOME/.config/hypr/README.md" '# Hyprland Advanced Configuration (Merged)
...
## Installation
```bash
chmod +x hyprland-advanced-extras-merged.sh
./hyprland-advanced-extras-merged.sh
```
...
```
This will back up the current configuration, delete the existing `~/.config/hypr/` directory, create a fresh one, and optionally re-run the installation script if its path is recorded in `~/.config/hypr/install_script_path`.
...
log_success "Hyprland Advanced Extras (Merged) installation complete" 
